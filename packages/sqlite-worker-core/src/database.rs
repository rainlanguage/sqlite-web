...
mod tests {
    ...
    async fn test_opfs_initialization_success() {
        ...
        assert!(result.is_ok(), "OPFS database initialization should succeed");
        let db = result.unwrap();
        assert!(!db.db.is_null(), "Database pointer should not be null after initialization");
    }
    ...
    async fn test_create_table_and_insert() {
        ...
        assert!(create_result.is_ok(), "CREATE TABLE should execute successfully");
        assert!(create_result.unwrap().contains("Rows affected: 0"), "CREATE TABLE should report 0 rows affected");

        let insert_result = db.exec("INSERT INTO test_users (name, age) VALUES ('Alice', 25)").await;
        assert!(insert_result.is_ok(), "INSERT statement should execute successfully");
        assert!(insert_result.unwrap().contains("Rows affected: 1"), "INSERT should report 1 row affected");
    }
    ...
    async fn test_select_query_with_results() {
        ...
        let first = &array[0];
        assert_eq!(first["name"].as_str().unwrap(), "Laptop", "First product name should be 'Laptop'");
        assert_eq!(first["price"].as_f64().unwrap(), 999.99, "First product price should be 999.99");
        
        let second = &array[1];
        assert_eq!(second["name"].as_str().unwrap(), "Mouse", "Second product name should be 'Mouse'");
        assert_eq!(second["price"].as_f64().unwrap(), 25.50, "Second product price should be 25.50");
    }
    ...
    async fn test_select_empty_result() {
        ...
        assert!(result.is_ok(), "SELECT from empty table should execute successfully");
        assert_eq!(result.unwrap(), "[]", "Empty SELECT should return empty JSON array");
    }
    ...
    async fn test_integer_column_handling() {
        ...
        assert_eq!(row["small_int"].as_i64().unwrap(), 42, "Small integer should be 42");
        assert_eq!(row["big_int"].as_i64().unwrap(), 9223372036854775807, "Large integer should be max i64 value");
    }
    ...
    async fn test_float_column_handling() {
        ...
        assert!((row["pi"].as_f64().unwrap() - 3.14159).abs() < 0.00001, "Pi should be approximately 3.14159");
        assert!((row["negative"].as_f64().unwrap() - (-2.71828)).abs() < 0.00001, "Negative float should be approximately -2.71828");
    }
    ...
    async fn test_text_column_handling() {
        ...
        assert_eq!(row["message"].as_str().unwrap(), "Hello World", "Text column should contain 'Hello World'");
        assert_eq!(row["empty"].as_str().unwrap(), "", "Empty text column should be empty string");
        assert!(row["null_val"].is_null(), "NULL text column should be null in JSON");
    }
    ...
    async fn test_blob_column_handling() {
        ...
        assert!(blob_str.starts_with("<blob"), "BLOB data should start with '<blob'");
        assert!(blob_str.contains("bytes>"), "BLOB data should contain 'bytes>''");
    }
    ...
    async fn test_column_names_handling() {
        ...
        assert!(row.get("full_name").is_some(), "Should have 'full_name' column");
        assert!(row.get("quoted col").is_some(), "Should have 'quoted col' column with spaces");
        assert_eq!(row["full_name"].as_str().unwrap(), "John Doe", "full_name should be 'John Doe'");
    }
    ...
    async fn test_update_query() {
        ...
        assert!(update_result.contains("Rows affected: 2"), "UPDATE should affect exactly 2 rows");
        ...
        assert_eq!(array[0]["value"].as_i64().unwrap(), 10, "First row value should remain 10");
        assert_eq!(array[1]["value"].as_i64().unwrap(), 40, "Second row value should be doubled to 40");
        assert_eq!(array[2]["value"].as_i64().unwrap(), 60, "Third row value should be doubled to 60");
    }
    ...
    async fn test_delete_query() {
        ...
        assert!(delete_result.contains("Rows affected: 2"), "DELETE should affect exactly 2 rows");

        let select_result = db.exec("SELECT COUNT(*) as count FROM test_delete").await.expect("Select failed");
        let parsed: serde_json::Value = serde_json::from_str(&select_result).expect("Invalid JSON");
        let array = parsed.as_array().expect("Should be array");
        assert_eq!(array[0]["count"].as_i64().unwrap(), 1, "Should have exactly 1 row remaining after delete");
    }
    ...
    async fn test_invalid_sql_syntax_error() {
        ...
        assert!(error.contains("Failed to prepare statement"), "Invalid SQL should produce prepare statement error");
    }
    ...
    async fn test_sql_with_null_byte_error() {
        ...
        assert!(result.is_err());
        let error = result.unwrap_err();
        assert!(error.contains("Invalid SQL string"), "SQL with null bytes should produce invalid string error");
    }
    ...
    async fn test_table_not_found_error() {
        ...
        assert!(result.is_err());
        let error = result.unwrap_err();
        assert!(error.contains("Query execution failed") || error.contains("no such table"), "Query on nonexistent table should fail with appropriate error");
    }
    ...
    async fn test_custom_functions_available() {
        ...
        assert!(row.get("result").is_some(), "Custom function should return a result");
    }
    ...
    async fn test_database_drop_cleanup() {
        {
            let Some(db) = get_test_db().await else { return };
            assert!(!db.db.is_null(), "Database pointer should be valid before drop");
        }
    }
    ...
    async fn test_multiple_statements_handling() {
        ...
        assert!(result.is_ok(), "Multiple statements should execute successfully");
    }
    ...
    async fn test_sequential_database_operations() {
        ...
        db.exec("CREATE TABLE sequential_test (id INTEGER PRIMARY KEY, value TEXT)").await.expect("Create failed");
        
        let insert1 = db.exec("INSERT INTO sequential_test (value) VALUES ('first')").await;
        let insert2 = db.exec("INSERT INTO sequential_test (value) VALUES ('second')").await;
        
        assert!(insert1.is_ok(), "First INSERT should succeed");
        assert!(insert2.is_ok(), "Second INSERT should succeed");
        
        let result = db.exec("SELECT COUNT(*) as count FROM sequential_test").await.expect("Select failed");
        let parsed: serde_json::Value = serde_json::from_str(&result).expect("Invalid JSON");
        let array = parsed.as_array().expect("Should be array");
        assert_eq!(array[0]["count"].as_i64().unwrap(), 2, "Should have exactly 2 rows after sequential inserts");
    }
}
