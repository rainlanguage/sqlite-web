...
pub unsafe extern "C" fn bigint_sum_step(
    ...
    if value_ptr.is_null() {
        // Skip NULL values (standard behavior for aggregate functions)
        return;
    }
    ...
    let aggregate_context = sqlite3_aggregate_context(context, std::mem::size_of::<BigIntSumContext>() as c_int);
    ...
    let bytes = std::slice::from_raw_parts(aggregate_context as *const u8, std::mem::size_of::<BigIntSumContext>());
    for &byte in bytes {
        if byte != 0 {
...
mod tests {
    ...
    fn test_bigint_sum_context_saturating_sub() {
        ...
        let very_large_negative = format!("-{}", U256::MAX.to_string());
